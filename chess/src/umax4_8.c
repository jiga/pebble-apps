...
#define J(A) K(y+A,b[(int)y])-K(x+A,u)-K(H+A,t)

#define U (1<<9)
static struct _ {int K,V;char X,Y,D;} A[U];           /* hash table, 16M+8 entries*/

static int M=136,S=128,I=8e3,Q,O,K,N,R,J,Z,k=16; /* M=0x88                */
...
int D(q,l,e,E,z,n)          /* recursive minimax search, k=moving side, n=depth*/
...
{                       /* e=score, z=prev.dest; J,Z=hashkeys; return score*/
 ...
 W((d++<n)||(d<3)||                                /* iterative deepening loop */
   ((z&(K==I))&&(((N<1e6)&(d<3))||                       /* root: deepen upto time   */
 ...
 {x=B=X;                                       /* start scan at prev. best */
  ...
  do{u=b[(int)x];                                   /* scan board looking for   */
   ...
   {r=p=u&7;                                   /* p = piece type (set r>0) */
    ...
    {A:                                        /* resume normal after best */
     ...
     do{                                       /* y traverses ray, or:     */
      ...
      t=b[(int)H];if((t&k)|((p<3)&(!((y-x)&7)-!t)))break;     /* capt. own, bad pawn mode */
      ...
      {v=p<6?b[x+8]-b[y+8]:0;                  /* center positional pts.   */
       b[(int)G]=b[(int)H]=b[(int)x]=0;b[(int)y]=u|32;             /* do move, set non-virgin  */
       ...
       {v-=9*((((x-2)&M)||b[x-2]-u)+               /* structure, undefended    */
        ...
        b[(int)y]+=V;i+=V;                          /* change piece, add score  */
       }
       ...
       b[(int)G]=k+6;b[(int)F]=b[(int)y]=0;b[(int)x]=u;b[(int)H]=t;     /* undo move,G can be dummy */
      }
...
int rand(void)
{
  ...
  return(((__seed = __seed * 214013L + 2531011L) >> 16) & 0x7fff);
}
